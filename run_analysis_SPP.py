...
for date in collected_results_orig:
    ...
    # if date > '2024-05-02' and date <= '2024-05-09':
    # if date > '2024-05-10' and date <= '2024-05-17':
    # if date > '2024-05-17' and date <= '2024-05-23':
    # if date > '2024-05-23' and date <= '2024-05-30':
...
# original_pnts = []
radius_for_points = [] 
titles = ['index', 'FP_for_chart', 'FN_for_chart', 'TP', 'FP', 'FN', 'Date', 'Monitored_uid', 'Contingency_uid', 'Num Hours', 'Rating', 'orig_rating', 'observed_RT_shadow_price', 'forecast_RT_shadow_price']
...
for date_of_forecast in collected_results:
    ...
    collected_results_summary.columns = ['counter', 'false_positive', 'false_negative', 'TP', 'FP', 'FN', 'tomorrow_date', 'monitored_uid', 'contingency_uid', 'num_points', 'rating', 'orig_rating', 'observed_RT_shadow_price', 'forecast_RT_shadow_price']
    ...
    for i in collected_results_summary[['counter', 'false_positive', 'false_negative', 'TP', 'FP', 'FN', 'tomorrow_date', 'monitored_uid', 'contingency_uid', 'num_points', 'rating', 'orig_rating', 'observed_RT_shadow_price', 'forecast_RT_shadow_price']].values.tolist():
        i[0] = counter
        counter += 1
...
if False:
    print(' ')
    print(f'OBSOLETE: this is not calculated anymore. Total RT shadow price for which both MUSE and Forecast did not reach {100 * min(percent_of_ratings[:-1])}% Rating: ${unidentified_RT_shadow_price:,.2f}')
    print(f'OBSOLETE: this is not calculated anymore. Percent of constraints both MUSE and Forecast did not reach {100 * min(percent_of_ratings[:-1])}% Rating: {(100 * unidentified_constraint_counter / (identified_constraint_counter + unidentified_constraint_counter)):,.1f}%')
    print(f'OBSOLETE: this is not calculated anymore. The RT shadow price associated with it: {(100 * unidentified_RT_shadow_price / (identified_RT_shadow_price + unidentified_RT_shadow_price)):,.1f}%')
print(' ')

i=0
for date_of_forecast in collected_results:
    for counter, false_positive, false_negative, TP, FP, FN, tomorrow_date, monitored_uid, contingency_uid, num_points, rating, orig_rating, observed_RT_shadow_price, forecast_RT_shadow_price in collected_results[date_of_forecast]:
        new_row = {'index':counter, 'FP_for_chart': false_positive, 'FN_for_chart': false_negative, 'TP': TP, 'FP': FP, 'FN': FN, 'Date': tomorrow_date, 'Monitored_uid': monitored_uid, 'Contingency_uid': contingency_uid, 'Num Hours': num_points, 'Rating': rating, 'orig_rating': orig_rating, 'observed_RT_shadow_price':observed_RT_shadow_price, 'forecast_RT_shadow_price':forecast_RT_shadow_price}
        weekly_info.loc[len(weekly_info)] = new_row
...
print(f'False Positive Cases where RT shadow prices = 0 but the Forecast is above the Rating: ({num_FP} in total)')
print(f'Total "made-up" Forecast RT shadow price in FP days: ${weekly_info.query("Rating == -1 and forecast_RT_shadow_price > 0 and observed_RT_shadow_price == 0").forecast_RT_shadow_price.sum():,.2f}')
print(weekly_info.query('Rating == -1 and FP_for_chart  == 1 and FN_for_chart == 0'))
print(' ')
print('MUSE fails on these PROBLEMATIC CONSTRAINTS:')
print(f"False Negative Cases, where actual RT shadow price > 0 but Forecast is less than {100 * min(percent_of_ratings[:-1]):.0f}% of the Rating: ({num_FN} in total)")
print(f'Total missed RT shadow price in FN days: ${weekly_info.query("Rating == -1 and FP_for_chart  == 0 and FN_for_chart == 1").observed_RT_shadow_price.sum():,.2f}')
...
max_forecast_minus_observed_RT_shadow_price = weekly_info.sort_values('difference', ascending=False).query('difference > 1000 and observed_RT_shadow_price < 1')
min_forecast_minus_observed_RT_shadow_price = weekly_info.sort_values('difference').query('difference < -3000 and forecast_RT_shadow_price < 1')
weekly_info = weekly_info.drop('difference', axis=1)
print(' ')
print(f'Rows with high FORECAST RT shadow price but low ACTUAL RT shadow price: (sum: ${max_forecast_minus_observed_RT_shadow_price.forecast_RT_shadow_price.sum():,.0f})')
...
print(' ')
print(f'Total observed_RT_shadow_price: ${weekly_info.query("Rating == -1").observed_RT_shadow_price.sum():,.2f}')
...
passed_on_RT_shadow_price = weekly_info.query("Rating == -1 and FP_for_chart  == 0 and FN_for_chart == 1").observed_RT_shadow_price.sum()
print(f'--> Total observed_RT_shadow_price in days that were passed on by the forecast (=false negative events): ${passed_on_RT_shadow_price:,.2f} out of ${weekly_info.query("Rating == -1").observed_RT_shadow_price.sum():,.2f} ({(100 * passed_on_RT_shadow_price / weekly_info.query("Rating == -1").observed_RT_shadow_price.sum()):.0f}%)')
print(' ')
print(f'Total forecast_RT_shadow_price: ${weekly_info.query("Rating == -1").forecast_RT_shadow_price.sum():,.2f}')
made_up_shadow_price = weekly_info.query("Rating == -1 and forecast_RT_shadow_price > 0 and observed_RT_shadow_price == 0").forecast_RT_shadow_price.sum()
print(f'--> Total forecast_RT_shadow_price in days that the Forecast predicted congestion, but actual RT shadow price was 0 the whole day (=false positive events): ${made_up_shadow_price:,.2f} out of ${weekly_info.query("Rating == -1").forecast_RT_shadow_price.sum():,.2f} ({(100 * made_up_shadow_price / weekly_info.query("Rating == -1").forecast_RT_shadow_price.sum()):.0f}%)')
print(' ')
diff_10PM_4AM = made_up_shadow_price - weekly_info.query("Rating == -1 and FP_for_chart  == 1 and FN_for_chart == 0").forecast_RT_shadow_price.sum()
print(f'--> Measure of instability: This is the $ ammount that the forecast for RT shadow price changed between 4AM and 10PM the day before: ${diff_10PM_4AM:,.2f}')
print(' ')
...
print(f'--> Ignoring "made-up" forecast RT shadow price, the % of real daily events that were ignored by Forecast is {percent_events_ignored:.0f}%')
print(f'--> Ignoring "made-up" forecast RT shadow price, the % of real daily events that were detected by Forecast is {percent_events_detected:.0f}%')
print(' ')
...
def create_plot(Radius_to_unify_points):
    ...
    def find_FP_cluster():
        ind = 0
        for FP, FN, TP in zip(daily_confusion_FP, daily_confusion_FN, daily_confusion_TP):
    ...
    for ind_rating, percent_of_rating in enumerate(percent_of_ratings):
        ...
        for ind, chosen_function in enumerate(collection_of_criteria):
            ...
            for i in range(len(daily_confusion_FP)):
                ...
                if ind_of_FP_blob == i:
                    ax.text(np.sqrt(daily_confusion_FP[i]), np.sqrt(daily_confusion_FN[i]), f'{values[i]:.0f}', fontsize=text_size[i], ha='center', va='center', color='black')
            ...
            if chosen_function == weight_on_num_of_active_hours_per_day:
                ...
                if False:
                    container = ax.bar([1-bin for bin in bins][-1::-1][:-1], hist[-1::-1], width=np.abs(np.diff([1-bin for bin in bins][-1::-1])), align='edge', edgecolor='black', color=['green','orange','red'], alpha=0.5)
                else:
                    total_height = hist[-1::-1]
                    ind_of_FP_blob = find_FP_cluster()
                    percent_of_FP_out_of_all_observations = values[ind_of_FP_blob] / sum(values)
                    bottom_height = total_height * np.array([1,1,1]) / (1 - percent_of_FP_out_of_all_observations)
                    ...
                    container = ax.bar([1-bin for bin in bins][-1::-1][:-1], bottom_height, width=np.abs(np.diff([1-bin for bin in bins][-1::-1])), align='edge', edgecolor='black',
                                        color=['green','orange','red'], alpha=0.5)

                    ax.bar([1-bin+.1 for bin in bins][-1::-1][:-1], top_height, width=np.abs(np.diff([1-bin for bin in bins][-1::-1])), align='edge', edgecolor='black',
                            color=['green','orange','black'], alpha=0.5)

    
            if not chosen_function == weight_on_num_of_active_hours_per_day:
                ax.set_xticks([.25, .7, .95, 1.05])
                ax.set_xticklabels(['TP > 0.5', '0.5-0.9', '< 0.1', ' FP'], rotation=-45, fontsize=16)
            else:
            ...
            df_historic_actual = pd.DataFrame(past_histogram_printout_collection, columns=titles)
            
            # remove duplicates in df_historic_actual
            df_historic_actual = df_historic_actual.drop_duplicates(subset=['title', 'Rating', 'bin', 'value', 'date_begining', 'date_ending'])
            ...
            for this_bin, this_hist_value in zip([1-bin for bin in bins][-1::-1], hist[-1::-1]):
                ...
                relevant_values = df_historic_actual.query('title == @title and Rating == @percent_of_rating and bin == @this_bin')['value']
            ...
            if chosen_function == weight_on_num_of_active_hours_per_day:
                ...
                ax.errorbar([1 - bin for bin in bin_midpoints][-1::-1], bottom_height, yerr=yerr, fmt='none', capsize=5, color='black')
            
            if title in ["weight: num of days in the blob", "weight: number of hours in the blob"] and percent_of_rating == -1:
                ...
                df_historic_FP = pd.DataFrame(FP_historic_collection, columns=titles_FP)
                df_historic_FP = df_historic_FP.drop_duplicates(subset=['value', 'date_begining', 'date_ending'])

                # find the percentile of this_hist_value in the relevant_values_FP by assuming a normal distribution
                FP_percentage_normalized = np.round(100 * FP_percentage / (FP_percentage + 100))
                quantile_FP = norm.cdf(FP_percentage_normalized, loc=np.mean(relevant_values_FP), scale=np.std(relevant_values_FP))
                FP_historic_collection.append([title, np.round(FP_percentage_normalized *100)/100, date_begining, date_ending])

                if title == "weight: num of days in the blob" and percent_of_rating == -1:
                    FP_stats["weight: num of days in the blob"] = {"value":FP_percentage_normalized, "quantile": np.round(100*quantile_FP), "Mean": np.round(np.mean(relevant_values_FP))}
                elif title == "weight: number of hours in the blob" and percent_of_rating == -1:
                    FP_stats["weight: number of hours in the blob"] = {"value": FP_percentage_normalized, "quantile": np.round(100*quantile_FP), "Mean": np.round(np.mean(relevant_values_FP))}
    ...
    df_historic_actual = df_historic_actual.drop_duplicates(subset=['title', 'Rating', 'bin', 'value', 'date_begining', 'date_ending'])
    df_historic_FP = df_historic_FP.drop_duplicates(subset=['title', 'value', 'date_begining', 'date_ending'])

    print(f'--> False Positive % for number of days: {FP_stats["weight: num of days in the blob"]["value"]:.0f}%  |   {FP_stats["weight: num of days in the blob"]["quantile"]:.0f}-th quantile of the historic data  |  Historic mean: {FP_stats["weight: num of days in the blob"]["Mean"]:.0f}')
    print(f'--> False Positive % for number of hours: {FP_stats["weight: number of hours in the blob"]["value"]:.0f}%  |  {FP_stats["weight: number of hours in the blob"]["quantile"]:.0f}-th quantile of the historic data  |  Historic mean: {FP_stats["weight: number of hours in the blob"]["Mean"]:.0f}')
