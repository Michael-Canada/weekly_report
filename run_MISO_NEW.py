...
Num_of_days_to_look_back = 23  #9
percent_of_ratings = [0.99, 0.95, 0.9, 0.85, 0.8]
# percent_of_ratings = [0.99, .8]
...
for as_of in tqdm(as_ofs_actual):
    ...
    url_FP = f"https://api1.marginalunit.com/pr-forecast/{run}/binding_constraint?as_of={date.strftime('%Y-%m-%d')}T04%3A00%3A00-05%3A00&resample=H&include_empty_timestamps=true"
...
with open('bc_MISO_actual_as_of.pkl', 'wb') as f:
...
for as_of in tqdm(as_ofs_actual):
    ...
    for monitored_uid, contingency_uid  in top_contingencies.index: 
        ...
        url = f'https://api1.marginalunit.com/pr-forecast/{run}/constraint?as_of={from_date}T04:00:00-05:00&as_of_match=nearest_before&monitored_uid={monitored_uid}&contingency_uid={contingency_uid}'  #forecast future days
        content = _get_dfm(url)
        if content is None:
        ...
        if not len(FORECAST_constraint_flow_tomorrow) == 24  or not len(MUSE_constraint_flow_tomorrow) == 24:
            ...
            print('Issue:  Expected 24 hours of data. Filling up missing forecast constraint_flow with zeros.   932849287394872')
            
            # Assuming 'timestamp_column_name' is the name of your timestamp column
            timestamp_column_name = 'timestamp'

            # Convert the timestamp column to datetime format
            FORECAST_constraint_flow_tomorrow[timestamp_column_name] = pd.to_datetime(FORECAST_constraint_flow_tomorrow[timestamp_column_name])

            # Determine the minimum and maximum dates
            min_date = FORECAST_constraint_flow_tomorrow[timestamp_column_name].dt.date.min()
            max_date = FORECAST_constraint_flow_tomorrow[timestamp_column_name].dt.date.max()

            # Generate a complete datetime range covering every hour between the min and max dates
            full_datetime_range = pd.date_range(start=min_date, end=max_date + pd.Timedelta(days=1), freq='H', closed='left')

            # Create a DataFrame from this complete datetime range
            df_full_range = pd.DataFrame(full_datetime_range, columns=[timestamp_column_name])

            # Merge the new DataFrame with the original, ensuring all hours are included
            FORECAST_constraint_flow_filled = pd.merge(df_full_range, 
            ...
            FORECAST_constraint_flow_filled['constraint_flow'] = FORECAST_constraint_flow_filled['constraint_flow'].fillna(0)

            # the following line fills up the rating values that are missing with the previous value
            FORECAST_constraint_flow_filled['rating'] = FORECAST_constraint_flow_filled['rating'].ffill()
        ...
        for percent_of_rating in percent_of_ratings:
            ...
            nan_constraint_flow = FORECAST_constraint_flow_tomorrow['constraint_flow'].isnull()
            ...
            if nan_constraint_flow.any():
                print("Rows with NaN were found in 'constraint_flow'.   883847928387483")
                break
...
with open(f'weekly_report_saved_data_MISO_NEW_as_of_{today_date}.pkl', 'wb') as f:
