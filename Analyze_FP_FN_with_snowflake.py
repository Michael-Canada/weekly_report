...
if ISO_name == "miso":
    schema = "miso_20241023T1104Z"
elif ISO_name == "ercot":
    # schema = "ercot_20240909T1130Z"  # ercot
    schema = "ercot_20241127T1120Z"  # ercot_prt_crcl
elif ISO_name == "spp":
...
if False:
    ...
    url = f"{CONSTRAINTDB_ROOT}/{ISO_name}/rt/binding_events?start_datetime={date_first}&end_datetime={date_last}"
...
for as_of in tqdm(as_ofs_actual[-1::-1]):
    ...
    if USE_SNOWFLAKE:
        ...
        # and as_of = '2024-11-07 04:00:00.000 -0600'
        # and DATE(timestamp) = '2024-11-09'
        # and shadow_price > 0
        # order by shadow_price"""

        # https://github.com/enverus-pr/mu-placebo-api/blob/master/placebo_api/db/snowflake/binding_constraint.py
        # line 81 (def get_binding_constraints() in mu-placebo-api/placebo_api/db/snowflake/binding_constraint.py)
        if WITH_IIR:
            Summer_query = f"""
            select stream_uid, timestamp, monitored_uid, contingency_uid, shadow_price, from_station, from_kv, to_station, to_kv
            from "binding_constraint" 
            where stream_uid = 'load_100_wind_100_sol_100_ng_100_iir' 
            AND HOUR(as_of) = 4 
            ...
            select stream_uid, timestamp, monitored_uid, contingency_uid, shadow_price, from_station, from_kv, to_station, to_kv
            from "binding_constraint" 
            where stream_uid = 'load_100_wind_100_sol_100_ng_100' 
            AND HOUR(as_of) = 4 
            and DATE(as_of) = '{date.strftime("%Y-%m-%d")}'
            and shadow_price > 0 
            order by shadow_price"""
        ...
        # df = snowflake_utils.execute_and_fetch(
        #     cnx,
        #     """select count(*) from "binding_constraint" where HOUR(as_of) = 4 and stream_uid = 'load_100_wind_100_sol_100_ng_100_iir' and as_of = '2024-11-07 04:00:00.000 -0600' and DATE(timestamp) = '2024-11-08' and shadow_price > 0 order by shadow_price""",
        # )
        ...
        df_bc_actual_FP = snowflake_utils.execute_and_fetch(cnx, Summer_query)

        df_bc_actual_FP.timestamp = df_bc_actual_FP.timestamp.dt.tz_convert(
            ISO_TO_TZ[ISO_name]
        )
        df_bc_actual_FP["as_of"] = df_bc_actual_FP.timestamp.dt.tz_convert(
            ISO_TO_TZ[ISO_name]
        )
        ...
        if False:
            location = "pr-forecast/ercot_20240909T1130Z"
            snowflake_user = os.environ["SNOWFLAKE_USER"]  # msimantov
            key_path = os.environ["SNOWFLAKE_PRIVATE_KEY_PATH"]

            if True:  # use private non-encrypted key:
                snowflake_cnx = f"snowflake://{snowflake_user}@enverus_pr.us-east-1/{location}?privatekeyfile={key_path}"
            else:  # use private key
                snowflake_password = os.environ["SNOWFLAKE_PRIVATE_KEY_PASSWORD"]
                snowflake_cnx = f"snowflake://{snowflake_user}:{snowflake_password}@enverus_pr.us-east-1/{location}"

            snowflake_ctx_params = snowflake_utils.ConnectionParams.from_cnx_string(
                snowflake_cnx
            )

            # snowflake_auth = os.environ["SNOWFLAKE_AUTH"]
            # snowflake_cnx = f"snowflake://{snowflake_auth}@enverus_pr.us-east-1/{location}"
            # snowflake_ctx_params = snowflake_utils.ConnectionParams.from_cnx_string(
            #     snowflake_cnx
            # )
            snow_conn = snowflake_ctx_params.build()
            # dfm = snowflake_utils.execute_and_fetch(snow_conn, query, params)
            dfm = snowflake_utils.execute_and_fetch(snow_conn, query)
            # dfm = process_forecast_lmp(iso, dfm)

        # # ############################
        # query = """
        # SELECT count(*)
        # FROM `pr-forecast.ercot_20240909T1130Z.binding_constraint`
        # WHERE HOUR(as_of) = 4
        # AND stream_uid = 'load_100_wind_100_sol_100_ng_100_iir'
        # /*AND as_of > '2024-01-01'*/
        # AND as_of = '2024-11-07 04:00:00.000 -0600'
        # AND DATE(timestamp) = '2024-11-08'
        # AND shadow_price > 0
        # order by shadow_price
        # """
        # location = "pr-forecast/ercot_20240909T1130Z"
...
with open(f"all_top_contingencies_{run.upper()}_NEW_test.pkl", "wb") as f:
