...
if SAVE_DATA:
    ...
    if USE_PERCENTILE:
        ...
        dff = pd.read_parquet(f"{ISO}_aggregated_data_{today_date}_flow.parquet")
        # dff = pd.read_parquet(f"{ISO}_aggregated_data_2024-12-04_flow.parquet")
    ...
    if not merged_df_hr.empty:
        ...
        merged_df_all_data.drop(("spline1", ""), axis=1, inplace=True)
        merged_df_all_data.drop(("spline2", ""), axis=1, inplace=True)
