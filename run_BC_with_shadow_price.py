...
# See if backfill is done:
# curl -u $SELF -s https://api1.marginalunit.com/pr-forecast/spp-1009/as_ofs | grep 2023-01-02

# Check what runs exist:
# curl -u $SELF -s https://api1.marginalunit.com/pr-forecast/runs
...
min_analysis_date = "2024-11-29"
max_analysis_date = "2024-12-06"
...
as_ofs_actual = set(as_ofs_last_year)
as_ofs_actual = [
    AsOf(
...
for as_of in tqdm(as_ofs_actual):
    ...
    date_for_daylight_saving = timezone.localize(date_strp)

    # check if it is in daylight saving time
    is_dst = bool(date_for_daylight_saving.dst())
    if run_for_actual_shadow_prices == "miso":
        is_dst = True
    ...
    if content is None:
        ...
        continue
    df_actual_one = process_actual(content)
    contents_actual[as_of.as_of_str[:10]] = df_actual_one
    ...
    if is_dst:
        ...
        url_FP = f"https://api1.marginalunit.com/pr-forecast/{run}/binding_constraint?as_of={date.strftime('%Y-%m-%d')}T05%3A00%3A00-05%3A00&resample=H&include_empty_timestamps=true&stream_uid=load_100_wind_100_sol_100_ng_100_iir"
...
def find_top_congestions(
    ...
    congestions_with_interest = merged_df[
        (merged_df["forecast_shadow_price"] > 50) | (merged_df["shadow_price"] > 0)
    ]
...
for as_of in tqdm(as_ofs_actual):
    ...
    for monitored_uid, contingency_uid in top_contingencies.index:
        ...
        if is_dst:
            ...
            url = f"https://api1.marginalunit.com/pr-forecast/{run}/constraint?as_of={from_date}T05:00:00-05:00&as_of_match=nearest_before&stream_uid=load_100_wind_100_sol_100_ng_100_iir&monitored_uid={monitored_uid}&contingency_uid={contingency_uid}"  # forecast future days
        content = _get_dfm(url)
        if content is None:
...
with open(
    f"weekly_report_saved_data_{run}_as_of_{today_date}.pkl",
    "wb",
) as f:
