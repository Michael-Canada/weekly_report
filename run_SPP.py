...
for as_of in tqdm(as_ofs_actual):
    ...
    for monitored_uid, contingency_uid in top_contingencies.index:
        ...
        if (
            not len(FORECAST_constraint_flow_tomorrow) == 24
            or not len(MUSE_constraint_flow_tomorrow) == 24
        ):
            if (
                len(FORECAST_constraint_flow_tomorrow) == 0
                or len(MUSE_constraint_flow_tomorrow) == 0
            ):
                continue  # no data for this day
            print(
                "Issue:  Expected 24 hours of data. Filling up missing forecast constraint_flow with zeros.   932849287394872"
            )
